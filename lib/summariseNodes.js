...
const summariseNodes = (nodes) => {
  ...
  return {
    total,
    fuji: _getTypeSummary(fuji),
    montBlanc: _getTypeSummary(montBlanc),
    kilimanjaro: _getTypeSummary(kilimanjaro),
    ushuaia: _getTypeSummary(ushuaia),
    everest: _getTypeSummary(everest),
    olympus: _getTypeSummary(olympus)
  }
}

module.exports = summariseNodes;
